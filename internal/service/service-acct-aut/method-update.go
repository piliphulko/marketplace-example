...
func (s *server) UpdateAccount(ctx context.Context, in *basic.AccountInfoChange) (*emptypb.Empty, error) {
	...
	if customer := in.GetCustomerChange(); customer != nil {
		// CUSTOMER
		if customer.CustomerAutOld.LoginCustomer == "" || &customer.CustomerAutOld.LoginCustomer == nil ||
		...
		// WAREHOUSE
		if warehouse.WarehouseAutOld.LoginWarehouse == "" || &warehouse.WarehouseAutOld.LoginWarehouse == nil ||
		...
		// VENDOR
		if vendor.VendorAutOld.LoginVendor == "" || &vendor.VendorAutOld.LoginVendor == nil ||
